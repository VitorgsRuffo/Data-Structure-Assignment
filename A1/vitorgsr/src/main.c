...
int main (int argc, char* argv[]){
    ...
    //Tratando o arquivo .geo:
        ...
        //(Os dados do geo serao organizados da seguinte maneira: matriz de ponteiros. Cala linha representa um comando, cada coluna uma parte deste comando.)
        ...
        char* command = (char*) malloc((command_max_length + 1) * sizeof(char));
        
        //if we have a nx in .geo we'll move the file cursor to the next line because we don't want to store the nx command.
        if(nx != 1000) fgets(command, command_max_length, geo);

        //Lendo o arquivo e preenchendo a matriz commands:
            
            int i = 0, commandLen;

            while(i < geo_lines_count){

                fgets(command, 100, geo);

                commandLen = strlen(command); //remover \n do final de cada comando:
                command[commandLen-1] = '\0';
 
                geo_interpret_command(command, commands, i);
        ...
        //Precisamos configurar a view box do svg que vai ser gerado para evitarmos que algumas figuras aparecam cortadas:
            ...
            char* openingSvgTag = (char*) malloc(70 * sizeof(char));
        ...
        //String que vai conter todas as tags referentes aos comandos do arquivo e que vai ser printada em um arquivo svg.
            char* svgFinalDocument = (char*) malloc((svgFinalDocMaxLen + 1) * sizeof(char));
        ...
        //Tratando um comando por vez: construir a tag relativa ao comando e anexa-la na string final.
    ...
    //Tratando o arquivo .qry (se necessário):  
        ...
        if(parameter.qryFileName != NULL){
            ...
            //String que vai conter todas as tags referentes aos comandos do arquivo e que vai ser printada em um arquivo svg.
                char* svgFinalDocumentQry = (char*) malloc((svgQryFinalDocMaxLen + 1) * sizeof(char));
            ...
            //String que vai conter todos textos referentes aos comandos do arquivo e que vai ser printada em um arquivo de saida txt.
                char* txtFinalContent = (char*) malloc((txtFinalContentMaxLen + 1) * sizeof(char));
            ...
            //Finalizando o .svg referente ao .qry:
                svg_append_content_to_final_document(closeTag, &svgFinalDocumentQry);
    ...
    //Finalizando .svg referente ao .geo:
        ...
        svg_append_content_to_final_document(closeTag, &svgFinalDocument);
